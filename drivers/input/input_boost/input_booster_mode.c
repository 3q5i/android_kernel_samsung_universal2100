...
void trigger_input_booster(struct work_struct *work)
{
	...
	if (p_IbTrigger->event_type == BOOSTER_ON) {
		...
		pr_booster_info(ITAG" IB Trigger Press :: IB Uniq Id(%d)\n", uniq_id);
		...
		mutex_lock(&ib->lock);
		pr_booster_info(ITAG" IB Trigger Release :: Uniq ID(%d)\n", ib->uniq_id);
		ib->rel_flag = FLAG_ON;
...
void press_state_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Press State Func :::: Unique_Id(%d) Head_Time(%d)\n",
...
void release_timeout_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Release Timeout Func :::: Unique_Id(%d)\n", target_ib->uniq_id);
	mutex_lock(&sip_rel_lock);
	for (res_type = 0; res_type < allowed_res_count; res_type++) {
		res = target_ib->ib_dt->res[allowed_resources[res_type]];
...
void remove_ib_instance(struct t_ib_info* target_ib)
{
	...
	if (!ib_exist) {
		...
		spin_unlock(&write_ib_lock);
		synchronize_rcu();
		pr_booster_info(ITAG" Del Ib Instance's Id : %d\n", target_ib->uniq_id);
		mutex_lock(&mem_lock);
		if (target_ib != NULL) {
			kfree(target_ib);
			target_ib = NULL;
...
int parse_device_info(struct device_node* np, struct t_ib_device_tree* ib_device_trees) {
	...
	for_each_child_of_node(np, cnp) {
		...
		for_each_child_of_node(resource_node, child_resource_node) {
			...
			pr_booster_info(ITAG" res_id(%d) result(%d)\n", res_id, result);
		...
		pr_booster_info(ITAG"ib_dt->label : %s\n", ib_dt->label);
...
//Init Cpu Cluster Value
	...
	pr_booster_info(ITAG" Init:: Total Cpu Cluster Count : %d\n", result);
	if (result < 0)
		goto out;
...
//Allow Resource
	...
	pr_booster_info(ITAG" Init:: Total Allow Resource Count: %d\n", result);
...
//Init Resource Release Values
	...
	pr_booster_info(ITAG" Init:: Total Release Value Count: %d\n", result);
	if (result == 0)
		goto out;
...
// Geting the count of sub boosters.
	...
	pr_booster_info(ITAG" %s : Total IB Mode Cnt : %d\n", __func__, num_of_mode);
...
// ib_boost_modes Init
	...
	for_each_child_of_node(np, cnp) {
		...
		pr_booster_info(ITAG"%s Total Input Device Count(%d)\n", ib_boost_modes[boost_mode].label, result);
	}
