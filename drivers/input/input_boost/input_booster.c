...
void trigger_input_booster(struct work_struct *work)
{
	...
	if (p_IbTrigger->event_type == BOOSTER_ON) {
		...
		pr_booster_info(ITAG" IB Trigger Press :: IB Uniq Id(%d)", uniq_id);
		...
		mutex_lock(&ib->lock);
		pr_booster_info(ITAG" IB Trigger Release :: Uniq ID(%d)", ib->uniq_id);
		ib->rel_flag = FLAG_ON;
...
void press_state_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Press State Func :::: Unique_Id(%d)", target_ib->uniq_id);
...
void press_timeout_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Press Timeout Func :::: Unique_Id(%d) Tail_Time(%d)",
...
void release_state_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Release State Func :::: Unique_Id(%d) Rel_Flag(%d)",
...
void release_timeout_func(struct work_struct* work)
{
	...
	pr_booster_info(ITAG" Release Timeout Func :::: Unique_Id(%d)", target_ib->uniq_id);
	mutex_lock(&sip_rel_lock);
	for (res_type = 0; res_type < allowed_res_count; res_type++) {
		res = target_ib->ib_dt->res[allowed_resources[res_type]];
...
void remove_ib_instance(struct t_ib_info* target_ib)
{
	...
	if (!ib_exist) {
		...
		spin_unlock(&write_ib_lock);
		synchronize_rcu();
		pr_booster_info(ITAG" Del Ib Instance's Id : %d", target_ib->uniq_id);
		mutex_lock(&mem_lock);
		if (target_ib != NULL) {
			kfree(target_ib);
			target_ib = NULL;
...
// Geting the count of devices.
	...
	pr_booster_info(ITAG" %s   ndevice_in_dt : %d\n", __func__, ndevice_in_dt);
...
// Get Needed Information from dtsi
	...
	pr_booster_info(ITAG" resource size : %d, cluster count : %d",
...
//Init Cpu Cluster Value
	...
	pr_booster_info(ITAG" Init:: Total Cpu Cluster Count : %d", result);
	if (result < 0)
		goto out;
...
//Allow Resource
	...
	pr_booster_info(ITAG" Init:: Total Allow Resource Count: %d", result);
...
//Init Resource Release Values
	...
	pr_booster_info(ITAG" Init:: Total Release Value Count: %d", result);
	if (result < 0)
		goto out;
	...
	for_each_child_of_node(np, cnp) {
		...
		pr_booster_info(ITAG" %s   ib_dt->label : %s\n", __func__, ib_dt->label);
	...
	pr_booster_info(ITAG" Total Input Device Count(%d), IsSuccess(%d)", device_count, ib_init_succeed);
